<x-user-layout>
    ...
    <section id="products-section" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14" x-data="{
        ...
        async fetchProducts() {
            ...
            if (this.activeBrand) params.set('brand', this.activeBrand);
        ...
        filter(category) {
            ...
            this.activeBrand = ''; // Reset brand filter when category changes
        ...
        <div class="flex flex-wrap items-center justify-center gap-3 mb-12">
            @foreach ($categories as $cat)
                ...
                <button type="button" @click="filter('{{ $cat['name'] }}')"
                    class="relative h-[56px] min-w-[140px] inline-flex items-center justify-center gap-2.5 px-8
                           rounded-full text-sm font-bold border transition-all duration-500
                           active:scale-95 disabled:opacity-70 group"
                    :class="activeCategory === '{{ $cat['name'] }}'
                        ? 'bg-blue-600 text-white border-blue-600 shadow-[0_15px_30px_-10px_rgba(37,99,235,0.4)]' 
                        : 'bg-white text-gray-500 border-gray-100 hover:border-blue-200 hover:text-blue-600 shadow-sm hover:shadow-md'"
                    :disabled="isLoading">
                    
                    <span :class="{ 'opacity-20': isLoading && loadingCategory === '{{ $cat['name'] }}' }"
                        ...
                        <div class="w-7 h-7 flex items-center justify-center transition-transform group-hover:scale-110">
                            @if ($displayIcon)
                                <img src="{{ $displayIcon }}" alt="{{ $cat['name'] }}" class="w-full h-full object-contain"
